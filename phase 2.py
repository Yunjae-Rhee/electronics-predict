...
wx  = pd.read_csv(f'{DATA_DIR}/weather.csv')         # date, country, avg_temp, ...
...
# --- 거시/노동 ---
cci = pd.read_csv(f'{DATA_DIR}/consumer_confidence.csv')  # (옵션) 수요 예측용
...
# --- 모드 메타 (먼저 선언: 이후 거리/리드타임/허용성 계산에서 사용) ---
...
TRUCK_BASE_CO2_PER_KM  = 0.40  # co2_per_km_factor가 배수라면 이 값과 곱함
...
# 도시 좌표: 공장 좌표(여러 개면 평균), 없으면 창고로 보강
...
if miss:
    ...
    for j in list(miss):
        if j in wh_city_latlon.index:
            ...
            miss.remove(j)
    if miss:
        raise ValueError(f"[도시 좌표 없음] {miss} → 해당 도시에 공장/창고 좌표 필요")
...
fx_long['usd_per_local'] = 1.0 / fx_long['usd_to_local']  # USD/LOCAL
...
for k in K:
    for s in SKUS:
        for t in dates:
            ...
            I_prev = (2000 * x_wh[k]) if (t == DATE_START) else Ivar[k,s,t - timedelta(days=1)]
...
cost_change = 0.0  # TODO: 4주 고정 변경수수료(5%) 선형화 가능
