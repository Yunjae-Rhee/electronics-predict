...
for k in K:
    for s in SKUS:
        for t in dates:
            ...
            I_prev = (2000 * live_wh[k, DATE_START]) if (t == DATE_START) \
                     else Ivar[k,s,t - timedelta(days=1)]
...
if m.status in [GRB.OPTIMAL, GRB.TIME_LIMIT]:
    ...
    for i in I:
        for s in SKUS:
            for t in dates:
                v = int(round(P[i,s,t].X))
                if v > 0:
                    prod_rows.append([t.strftime('%Y-%m-%d'), 'factory', s, v, int(round(H_ot[i,t].X)), 0, i, None, None])
        ...
        for s in SKUS:
            for t in dates:
                for m_ in modes:
                    ...
                    if qty > 0:
                        ship_rows.append([t.strftime('%Y-%m-%d'),'factory',s,0,0,qty, i, k, m_])
    for (k,j) in KJ:
        for s in SKUS:
            for t in dates:
                for m_ in modes:
    ...
    built = []
    for i in I:
        if x_fac_any[i].X > 0.5:
            built.append(['factory', i, site_country[i], site_city[i], site_lat[i], site_lon[i]])
    for k in K:
        if x_wh_any[k].X > 0.5:
            built.append(['warehouse', k, site_country[k], site_city[k], site_lat[k], site_lon[k]])

    built_df = pd.DataFrame(built, columns=['site_type','site_id','country','city','lat','lon'])
    built_df.to_csv('selected_sites.csv', index=False)
    ...
    for i in I:
        for t in dates:
            if b_fac[i,t].X > 0.5:
                tl.append(['factory', i, 'build', t.strftime('%Y-%m-%d')])
            if live_fac[i,t].X > 0.5:
                tl.append(['factory', i, 'live',  t.strftime('%Y-%m-%d')])
    for k in K:
        for t in dates:
            if b_wh[k,t].X > 0.5:
                tl.append(['warehouse', k, 'build', t.strftime('%Y-%m-%d')])
            if live_wh[k,t].X > 0.5:
                tl.append(['warehouse', k, 'live',  t.strftime('%Y-%m-%d')])
    if tl:
        timeline_df = pd.DataFrame(tl, columns=['site_type','site_id','event','date'])
        timeline_df.to_csv('site_timeline.csv', index=False)